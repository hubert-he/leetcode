...
func RunArray(){
	t1 := []int{2, 7, 11, 15}
	target := 9
	fmt.Println(twoSum(t1, target))
	t3 := []int{3,2,3}
	target = 6
	fmt.Println(twoSumTwoPointer(t3, target))

	fmt.Println(twoThing(t1, target, difference))

	t2 := []int{-1, 0, 1, 2, -1, -4}
	target = 0
	fmt.Println(threeSum(t2, 0))

	t4 := []int{0, 0, 0, 0}  // 检查重复元素
	fmt.Println(commonSum(t4,0, 3))
}
